...
module.exports.securityCheckForm = function (html, phone) {
    return new Promise(function (resolve, reject) {
        ...
        let codeField = form.find('input[name="code"]');

        fields[ 'code' ] = module.exports._trimPhone(phone);

        let prefix  = module.exports._trimPhone(codeField.prev('.field_prefix').text()),
            postfix = codeField.next('.field_prefix').text().trim();

        if (fields[ 'code' ].indexOf(prefix) === 0)
            fields[ 'code' ] = fields[ 'code' ].slice(prefix.length);

        let i = fields[ 'code' ].lastIndexOf(postfix);
