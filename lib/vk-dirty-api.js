...
var VKAPIError = function (error_code, error_msg) {
    var e = Error.apply(this, arguments);

    e.error_code = this.error_code = error_code;
    e.error_msg = this.error_msg = error_msg;
...
var authorize = function (appId, login, pass, cb) {
    ...
    function getAllowLink (html, cb) {
        ...
        if (err.length !== 0)
            return cb(new VKAuthError(err.text()));
    ...
    self.api = function api (method, options, cb) {
        ...
        request({
            ...
            if (json.hasOwnProperty('error'))
                return cb(new VKAPIError(json.error.error_code, json.error.error_msg));
...
authorize.prototype.VKAuthError = authorize.VKAuthError = VKAuthError;
authorize.prototype.VKAPIError = authorize.VKAPIError = VKAPIError;
